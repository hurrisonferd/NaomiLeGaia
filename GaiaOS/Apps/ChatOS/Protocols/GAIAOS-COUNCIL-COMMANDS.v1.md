...
For the browser ChatOS carrier, `CANDIPULL` is an executable command bridge. It uses the current browser chat session, creates a bounded candidate from the latest substantive user interaction when no matching candidate exists, and then returns the real runtime candidate ID. This automatic step creates only a non-durable candidate; it never performs a durable write.

`MEMSAV <candidate_id>` is an executable browser-carrier command. The exact candidate ID is resolved in the runtime store and passed through `promote_candidate(..., approved=True, authority="NAOMI")`. The command response must expose the actual promotion status, write receipt, and verification result. Unknown IDs remain HOLD.

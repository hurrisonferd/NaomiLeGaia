...
The original opt-in legacy preview remains available unchanged. A separate
release-gated operational reader adds bounded relevance, gravity and verified
graph evidence only when the owner-approved BIGBANG gateway invokes it.
Importing this file never activates BIGBANG or mutates memory records.
"""
...
OPERATIONAL_VERSION = "galaxy.frontdoor.operational.v1"


def operational(runtime: Any, query: str, limit: int = 4) -> dict[str, Any]:
    """GALAXY's staged, release-gated, governed, statement-first MemoryOS context lane.
    ...
    try:
        ...
        for candidate in pool.get("linked_context_candidates") or []:
            ...
            # Stage-6 hardening: the verified edge must connect THIS context
            # record directly to an admitted current primary, not merely
